...
#include <limits>    // For clearing invalid input buffer
#include <termios.h> // For password masking
#include <unistd.h>  // For password masking
...
private:
    ...
    bool isValidEmail(const string& email) {
        return regex_match(email, regex("^[\\w.-]+@(gmail\\.com|yahoo\\.com|outlook\\.com|hotmail\\.com|bank\\.com)$"));
    }

    bool isValidGender(const string& g) {
        string gen = g;
        for (auto& c : gen) c = tolower(c);
    ...
    void createNewAccount() {
        ...
        // Use password masking for account creation
        ...
        while (true) {
            cout << " Email (Gmail/Yahoo/Outlook): ";
            getline(cin, email);
            if (isValidEmail(email)) break;
            cout << " Invalid email! Use a valid domain (gmail.com, yahoo.com, outlook.com, etc.)\n";
        }
    ...
    // Login function with password masking
    bool loginUser() {
